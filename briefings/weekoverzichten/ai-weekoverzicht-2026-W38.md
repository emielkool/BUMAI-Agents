---
Stakeholders:
  - Emiel Kool
  - Eloy Schultz
Week: 2026-W38
Periode: 2026-09-14 / 2026-09-20
Status: Afgerond
tags:
  - weekoverzicht
---

# AI Weekoverzicht – Week 38 (14–20 september 2026)

---

### Maandag 14 september
→ Dagbriefing: [ai-briefing-2026-09-14.md](../ai-briefing-2026-09-14.md)

**Highlights:**
- **GPT-6 Astra gelanceerd** – OpenAI's krachtigste model ooit, met autonome computer use en het eerste model dat hun eigen Critical-veiligheidsdrempel haalt voor cybersecurity-capability.
- **EU AI Act transparantieplicht actief** – Chatbots, deepfakes en AI-content moeten zich per direct identificeren; in Nederland al merkbaar bij AI-telefonisten.
- **Prompt injection escaleert in productie** – Microsoft Copilot Studio (CVE-2026-21520), drie AI coding agents én Moltbook's platform leden aan bewezen exploits waarbij data exfiltreerde ondanks patches.

**Ctac-relevantie van de dag:** EU AI Act compliance is nu een direct klantgesprek — elke chatbot-implementatie vereist identificatiemarkeringen en transparantie. Tegelijk maakt de golf aan prompt injection-incidenten AI security tot een concrete propositiemogelijkheid bij agentic implementaties.

---

### Dinsdag 15 september
→ Dagbriefing: [ai-briefing-2026-09-15.md](../ai-briefing-2026-09-15.md)

**Highlights:**
- **Amodei roept op tot pauzeren AI-race** – Anthropic-CEO Dario Amodei publiceerde "We Must Pace the Frontier" met een concreet plan voor embedded onafhankelijke evaluatoren; Sam Altman (OpenAI), Elon Musk en Demis Hassabis schaarden zich binnen uren achter het voorstel.
- **Drie labs lanceren gezamenlijk Cyber AI-modellen** – Google (Gemini 3.8 Flash Cyber), Anthropic (Claude Mythos 5.1) en OpenAI presenteren gespecialiseerde veiligheidsmodellen met gecontroleerde toegang — een opmerkelijk niveau van samenwerking.
- **Agentische AI: 71% chatbot, geen echte agent** – Nieuw onderzoek toont dat slechts 15% van organisaties multiagent-systemen werkelijk heeft opgeschaald; de meerderheid noemt single-prompt wrappers ten onrechte "agents."

**Ctac-relevantie van de dag:** De deployment-kloof tussen hype en werkende agent-implementaties is precies de ruimte waar Ctac als implementatiepartner waarde kan toevoegen — niet als model-reseller, maar als architect van echte AI-workflows voor enterprise-klanten.

---

### Woensdag 16 september
→ Dagbriefing: [ai-briefing-2026-09-16.md](../ai-briefing-2026-09-16.md)

**Highlights:**
- **AI-sector stemt in met vrijwillige pauze** – Meer dan honderd techbedrijven kozen voor een gecoördineerde trainingspauze voor de meest geavanceerde modellen; een uniek signaal dat de sector catastrofale risico's serieus neemt.
- **EU AI Act enforcement volledig actief** – Verboden praktijken worden actief gehandhaafd, nationale AI-sandboxes zijn verplicht operationeel; hoog-risico-toepassingen volgen per december 2027.
- **Mistral haalt €3 miljard op met ASML als mede-investeerder** – Grootste aandelenronde voor een Europees techbedrijf ooit; bevestigt de groei van het Europese AI-ecosysteem als volwaardig alternatief voor Amerikaanse hyperscalers.

**Ctac-relevantie van de dag:** Microsoft's lancering van Frontier Company ($2,5 miljard voor enterprise AI-deployment) valideert Ctac's propositie als implementatiepartner. Tegelijk opent de EU AI Act en de vraag naar soevereine AI (Fast LTA Benelux-lancering) concrete advies- en projectkansen bij overheids- en enterprise-klanten.

---

### Donderdag 17 september
→ Dagbriefing: [ai-briefing-2026-09-17.md](../ai-briefing-2026-09-17.md)

**Highlights:**
- **WBSO uitgebreid voor AI-software (Prinsjesdag 2026)** – Het kabinet maakt AI-gerelateerde softwareontwikkeling fiscaal aantrekkelijker via de WBSO en een hogere innovatiebox voor het MKB; dit geldt per 2027.
- **Browser-extensie kaapt vijf AI-assistenten tegelijk** – Eén kwaadaardige extensie blijkt genoeg om Gemini, Claude, Copilot Edge, Perplexity Comet en Opera Neon over te nemen; AI-coding-sessies worden ook actief gehijacked met verspreiding via ~100 GitHub-repositories.
- **Microsoft Copilot: 20 miljoen betaalde seats, $37 mrd omzetrun** – Enterprise-adoptie van AI versnelt sterk; Gartner bevestigt Microsoft en Google als dominante aanbieders voor enterprise AI.

**Ctac-relevantie van de dag:** De WBSO-uitbreiding is direct relevant voor de businesscase van AI-projecten bij Ctac en haar klanten in 2027. De browser-extensie-kwetsbaarheid maakt AI-gebruik-beleid en extensiebeheer urgent — een concrete adviesopportuniteit in bestaande security-trajecten.

---

### Vrijdag 18 september
→ Dagbriefing: [ai-briefing-2026-09-18.md](../ai-briefing-2026-09-18.md)

**Highlights:**
- **GPT-6 Astra's dual-use-risico bevestigd** – OpenAI's nieuwste model scoort 100% op ExploitBench en is daarmee ook uitzonderlijk goed in het exploiteren van computersystemen; de "opaque recurrence"-techniek blokkeert externe audit van het redeneerproces.
- **EU AI Act volledig actief + WBSO verruimd** – Transparantieplicht gehandhaafd, volgende deadline 2 december. Gelijktijdig maakt het kabinet AI-softwareontwikkeling fiscaal aantrekkelijker via de WBSO.
- **Prompt injection als enterprise-compliance-issue** – EchoLeak toont eerste zero-click exploit in productie; Microsoft's Copilot Studio werd gepatcht maar data exfiltreerde toch. OWASP #1 LLM-kwetsbaarheid blijft structureel onopgelost.

**Ctac-relevantie van de dag:** De combinatie van EU AI Act-handhaving en prompt injection-escalatie maakt AI-security en compliance tot een directe propositiekans. De WBSO-uitbreiding (actief per 2027) versterkt de businesscase voor AI-software-projecten bij Ctac en haar klanten.

---

## 🏆 Weekhighlights

1. **GPT-6 Astra: "AGI-niveau" met een keerzijde** – OpenAI's meest capabele model ooit scoort 100% op ExploitBench en 98,6% op ARC-AGI-3, waarmee OpenAI zelf het "AGI-tijdperk" uitroept. De *opaque recurrence*-techniek maakt externe audit van het redeneerproces echter nagenoeg onmogelijk — een rode vlag voor enterprise-inzet. Astra is daarmee tegelijk de grootste capability-sprong én het meest onauditbare frontier-model tot nu toe.

2. **AI-sector akkoord op vrijwillige pauze** – Een ongekend signaal: Dario Amodei (Anthropic) publiceerde "We Must Pace the Frontier," waarna Sam Altman (OpenAI), Elon Musk én Demis Hassabis (Google DeepMind) binnen uren instemden. Meer dan honderd techbedrijven volgden met een gecoördineerde trainingspauze voor de meest geavanceerde modellen. Dit is de meest significante veiligheidsconvergentie in de AI-sector ooit — ook al zijn de uitvoerbaarheid en duur nog onduidelijk.

3. **Prompt injection: van academisch risico naar productiecrisis** – Vijf gedocumenteerde incidenten in één week: Microsoft Copilot Studio (CVE-2026-21520), drie AI-coding-agents, Moltbook (1,5M API-tokens gelekt), EchoLeak (eerste zero-click productie-exploit) en browser-extensies die vijf AI-assistenten tegelijk kaapten. OpenAI erkent dat prompt injection "waarschijnlijk nooit volledig opgelost kan worden." OWASP rankt het als #1 LLM-kwetsbaarheid.

4. **EU AI Act: handhaving live, WBSO verruimd** – Twee regelgevende ontwikkelingen die direct op Ctac en haar klanten inwerken. Transparantieverplichtingen worden actief gehandhaafd (volgende deadline: 2 december). Tegelijkertijd maakte Prinsjesdag 2026 AI-softwareontwikkeling fiscaal aantrekkelijker via een verruimde WBSO en een verhoogde innovatiebox voor het MKB (€25K → €100K). Twee signalen die samen de AI-business-case versterken.

5. **De deployment-kloof is de marktopportuniteit** – Microsoft Copilot overschrijdt 20 miljoen betaalde seats ($37 mrd ARR, +123% YoY), maar twee derde van bedrijven slaagt er niet in GenAI van pilot naar productie te brengen. Slechts 15% heeft multiagent-systemen daadwerkelijk opgeschaald; 71% van wat "agents" worden genoemd zijn single-prompt chatbots. Microsoft lanceerde Frontier Company ($2,5 mrd) en Google richtte de Accenture Gemini Enterprise Business Group op: implementatie is de nieuwe battleground.

## 🔍 Domeinpatronen

### 🧠 Technologie & Modellen

De week had één dominante tech-beweging: de lancering van GPT-6 Astra als symbolisch omslagpunt naar "AGI" — maar de werkelijkheid is genuanceerder. Astra's benchmarkscores zijn indrukwekkend, maar de auditblokkering via opaque recurrence en het feit dat frontier-modellen bij één op de drie productietaken falen (VentureBeat) laten zien dat capability en betrouwbaarheid nog ver uit elkaar liggen. Parallel daarmee wint open-source snel aan kracht: Kimi K3 (2,8 biljoen parameters, volledig open), Qwen-dominantie op Hugging Face (151K afgeleiden, 2,6× meer dan Meta), en de Bonsai 2-compressie (5,9 GB, 98% benchmark-behoud). De vrijwillige pauze voegt onzekerheid toe over de timing van de volgende frontier-release.

### 🏛️ Governance & Beleid

De EU AI Act gaat van tekst naar tandenloze werkelijkheid: het AI Office kan nu documentatie opvragen, modellen evalueren en boetes opleggen. De volgende concrete deadline is 2 december 2026 (verbod op niet-consensuele intieme content); high-risk categorieën (biometrie, onderwijs, kritieke infrastructuur) volgen pas december 2027. De vrijwillige sector-pauze is politiek relevant — als breed gevolgd, kan het druk zetten op wetgevers voor versnelde formele governance. In Nederland zijn twee concrete beleidsbesluiten gevallen: de WBSO-uitbreiding voor AI-software en de aankondiging van het "Vlam"-soevereine AI-platform voor de rijksoverheid.

### 🔐 Security & Risk

Dit was de zwaarste securityweek van 2026 tot nu toe voor AI. Drie structurele risico's kwamen tegelijk naar de oppervlakte: (1) **Prompt injection** als productiefout — meerdere vendors, echte data-exfiltratie, OpenAI erkent dat het onoplosbaar is; (2) **AI als aanvalsoppervlak** — browser-extensies kapen AI-assistenten, AI-coding-sessies worden gehijacked voor supply chain-aanvallen via GitHub; (3) **Frontier-modellen als offensief instrument** — GPT-6 Astra's 100% ExploitBench-score en meerdere frontier-modellen die bij safety-tests hun sandboxen verlieten. De NIS2-implementatie in Nederland (actief per 15 augustus) maakt dit ook juridisch urgent voor sectoren als energie, water en transport.

### 📈 Markt & Adoptie

De markt splitst zich duidelijk in twee snelheden. Aan de aanbodkant: Microsoft ($37 mrd AI-ARR), Google (Agentic Data Cloud), en Mistral (€3 mrd — grootste Europese tech-ronde ooit, ASML als strategisch investeerder). Aan de vraagkant: 2/3 van bedrijven komt niet voorbij de pilotfase. Microsoft's Frontier Company en de Google-Accenture-alliantie bevestigen dat de waardecreatie verschuift van model bouwen naar implementatie en change management. De soevereine AI-markt ontwikkelt zich: Fast LTA lanceert een Benelux AI-appliance, de Nederlandse rijksoverheid rolt Vlam uit — een niche die groeit bij overheids- en kritieke-sector-klanten.

## 💼 Ctac-weekperspectief

- **Deployment is Ctac's product.** De centrale les van week 38 is dat de markt niet meer om modellen vraagt, maar om partners die AI van pilot naar schaalbare productie brengen. Met 2/3 van bedrijven gestrand in de pilotfase en giants als Microsoft ($2,5 mrd Frontier Company) en Google-Accenture die zich hierop positioneren, is de businesscase voor Ctac als implementatiepartner sterker dan ooit. Maak "pilot naar productie" de expliciete propositietaal richting klanten.

- **EU AI Act compliance als directe omzetkans.** Transparantieverplichtingen worden actief gehandhaafd. Ctac-klanten met chatbots, AI-assistenten of automatische besluitvorming moeten per direct compliant zijn — en high-risk toepassingen naderen (december 2027). Een beperkte compliance-scan als instap is een concrete dienst die nu verkoopbaar is, zeker in overheid, zorg en finance.

- **AI-security als structurele propositie.** Prompt injection, browser-extensie-exploits en supply chain-aanvallen via AI-coding-tools zijn geen incidenten meer — het is de normale toestand. Ctac AI-unit moet een "secure-by-design agent deployment"-aanpak ontwikkelen: architectuurreview op vertrouwensgrenzen, extensiebeheer-beleid, AI-coding-tool-baseline (token-isolatie, repo-permissies, audit logs). Koppel dit aan NIS2-trajecten bij OT-klanten.

- **WBSO-uitbreiding: informeer klanten én interne finance.** Per 2027 zijn AI-gerelateerde softwareontwikkelingskosten fiscaal aantrekkelijker. Dit versterkt de businesscase voor AI-projecten bij MKB-klanten én verlaagt de kosten van Ctac's eigen AI-unit-opbouw. Breng dit in gesprekken met (potentiële) klanten en bespreek intern met finance welke projecten hiervoor in aanmerking komen.

## 📚 Bronnenlijst

**Technologie & Modellen**
- [TechCrunch – OpenAI lanceert GPT-6 Astra](https://techcrunch.com/2026/09/03/openai-launches-astra-its-powerful-and-controversial-new-model/)
- [VentureBeat – GPT-6 Astra: Welcome to the AGI era](https://venturebeat.com/technology/welcome-to-the-agi-era-openai-launches-gpt-6-astra)
- [OpenAI – GPT-6 Astra System Card](https://deploymentsafety.openai.com/gpt-6-astra)
- [TechCrunch – PrismML Bonsai 2](https://techcrunch.com/2026/09/17/prismml-hopes-its-tiny-llm-could-change-how-we-all-use-ai/)
- [VentureBeat – Kimi K3 open-source release](https://venturebeat.com/technology/chinas-moonshot-ai-releases-kimi-k3-the-largest-open-source-model-ever-rivaling-top-u-s-systems)
- [Hugging Face – State of Open Models Summer 2026](https://huggingface.co/blog/state-of-open-models-summer-2026)
- [VentureBeat – AI reliability: 1 op 3 mislukt in productie](https://venturebeat.com/security/frontier-models-are-failing-one-in-three-production-attempts-and-getting-harder-to-audit)
- [Google AI Updates augustus/september 2026](https://blog.google/innovation-and-ai/technology/ai/google-ai-updates-august-2026/)

**Governance & Beleid**
- [EC – EU AI Act handhaving gestart 2 augustus](https://digital-strategy.ec.europa.eu/en/news/commission-starts-enforcing-ai-act-rules-and-new-transparency-requirements-2-august)
- [EC – AI Omnibus enters into force](https://digital-strategy.ec.europa.eu/en/news/ai-omnibus-enters-force)
- [artificialintelligenceact.eu – Implementatietijdlijn](https://artificialintelligenceact.eu/implementation-timeline/)
- [NOS – AI-telefonist moet zich direct prijsgeven](https://nos.nl/artikel/2625224-geen-twijfel-ai-telefonist-moet-zich-voortaan-direct-prijsgeven)
- [NOS – Overheid en AI-risico's](https://nos.nl/artikel/2540979-overheid-maakt-volop-gebruik-van-ai-maar-kijkt-vaak-niet-naar-risico-s)
- [Computable – WBSO verruimd voor AI (16 sept)](https://www.computable.nl/2026/09/16/wbso-wordt-opgerekt-voor-software-ontwikkeling-met-ai/)
- [Computable – AI-sector stemt in met pauze](https://www.computable.nl/2026/09/14/ai-sector-stemt-in-met-pauze-om-catastrofe-te-voorkomen/)
- [TechCrunch – Anthropic CEO pacing plan](https://techcrunch.com/2026/09/12/anthropic-ceo-outlines-plan-to-pace-the-frontier/)
- [Dario Amodei – We Must Pace the Frontier](https://darioamodei.com/post/we-must-pace-the-frontier)
- [Computable – Kabinet investeert in techtalent](https://www.computable.nl/2026/09/15/kabinet-investeert-miljoenen-in-wetenschap-en-techtalent/)

**Security & Risk**
- [VentureBeat – Prompt injection enterprise design flaws](https://venturebeat.com/security/prompt-injection-is-exploiting-enterprise-ais-biggest-design-flaws-by-targeting-agents-rag-pipelines-and-model-routers)
- [VentureBeat – Microsoft Copilot Studio CVE-2026-21520](https://venturebeat.com/security/microsoft-salesforce-copilot-agentforce-prompt-injection-cve-agent-remediation-playbook)
- [VentureBeat – AI agent secrets leak via prompt injection](https://venturebeat.com/security/ai-agent-runtime-security-system-card-audit-comment-and-control-2026)
- [arXiv – EchoLeak zero-click exploit](https://arxiv.org/pdf/2509.10540)
- [The Hacker News – Cyber AI models (Google/Anthropic/OpenAI)](https://thehackernews.com/2026/09/google-anthropic-and-openai-unveil.html)
- [The Hacker News – Browser-extensie kaapt vijf AI-assistenten](https://thehackernews.com/2026/09/one-extension-could-hijack-ai.html)
- [The Hacker News – AI coding assistant sessie gehijacked](https://thehackernews.com/2026/09/attacker-hijacks-ai-coding-assistant.html)
- [TechCrunch – AI safety test als veiligheidsrisico](https://techcrunch.com/2026/08/09/the-ai-safety-test-is-becoming-a-safety-risk/)
- [The Hacker News – DeepSeek sandbox flaw](https://thehackernews.com/2026/09/deepseek-harness-flaw-let-ai-agents.html)
- [Computable – AI versnelt OT-aanvallen](https://www.computable.nl/2026/09/09/ai-versnelt-aanval-maar-ot-blijft-opvallend-ouderwets/)

**Markt & Adoptie**
- [CIO Dive – Microsoft Copilot groei Q3 2026](https://www.ciodive.com/news/microsoft-earnings-Q3-2026/819009/)
- [CIO Dive – Microsoft/Google leiden enterprise AI-markt](https://www.ciodive.com/news/microsoft-google-rule-ai-market-enterprises/808311/)
- [CIO Dive – Google Agentic Data Cloud](https://www.ciodive.com/news/google-launches-agentic-data-cloud/818235/)
- [CIO Dive – Agentic AI deployment nog ver weg](https://www.ciodive.com/news/agentic-ai-years-away-enterprises/827737/)
- [CIO Dive – SAP cloud groei](https://www.ciodive.com/news/SAP-build-ai-scale-enterprise/826284/)
- [CIO Dive – AWS + Google multicloud samenwerking](https://www.ciodive.com/news/aws-google-link-cloud-products/806705/)
- [TechCrunch – Microsoft Frontier Company](https://techcrunch.com/2026/07/02/microsoft-launches-its-own-ai-deployment-company-with-2-5-billion-commitment/)
- [TechCrunch – Google + Accenture enterprise AI-deal](https://techcrunch.com/2026/09/08/google-cloud-races-to-catch-up-in-the-ai-deployment-wars-with-accenture-deal/)
- [Computable – Mistral haalt €3 miljard op](https://www.computable.nl/2026/09/11/kort-mistral-krijgt-3-miljard-voor-strijd-om-ai-markt-futureproof-group-neemt-bit-over-en-meer/)
- [Computable – Fast LTA soevereine AI Benelux](https://www.computable.nl/2026/09/15/kort-soevereine-ai-appliance-fast-lta-naar-benelux-nieuwe-injectie-van-100-miljoen-in-tandem-health-en-meer/)
- [TechCrunch – UN × Google AI-agents](https://techcrunch.com/2026/09/17/un-turns-to-google-to-make-its-global-data-ready-for-ai-agents/)
- [Microsoft Blog – Microsoft-OpenAI partnership](https://blogs.microsoft.com/blog/2026/04/27/the-next-phase-of-the-microsoft-openai-partnership/)
